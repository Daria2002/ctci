...
std::vector<std::string> separate_string(const std::string& str, const std::string& regex) {
...
bool get_child(std::vector<std::pair<std::string, std::string>> dependency_pairs, Project child) {
    return true;
}
...
Graph build_graph(std::string project_names, std::string project_dependencies) {
    ...
    std::for_each(names.begin(), names.end(), [&](std::string name) {
        ...
        if(Project child; get_child(dependency_pairs, child)) {
